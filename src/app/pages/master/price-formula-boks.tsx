...
import { FileBox, Plus, Pencil, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { DataTable } from '../../components/data-table';

export default function PriceFormulaBoks() {
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      await api.deletePriceFormulaBoks(item.id);
  ...
  const columns = [
    {
      ...
      accessorKey: 'code',
      render: (value: string) => (
        <code className="text-xs bg-orange-50 px-2 py-1 rounded border border-orange-200">
          {value}
        </code>
      )
    },
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Formula Harga Boks"
        description="Daftar formula perhitungan harga untuk boks/kemasan karton"
        icon={FileBox}
        breadcrumbs={[
          { label: 'Dashboard', href: '/' },
          { label: 'Master' },
          { label: 'Formula Harga Boks' }
        ]}
      />
      ...
      <Card className="p-4 bg-orange-50 border-orange-200">
        <div className="flex items-start gap-3">
          <FileBox className="w-5 h-5 text-orange-600 mt-0.5" />
          <div>
            <h3 className="font-medium text-orange-900 mb-1">Formula Harga Boks</h3>
            <p className="text-sm text-orange-700">
              Digunakan untuk menghitung harga boks/kemasan karton (Duplex, Ivory, Kraft, Corrugated).
              Mencakup biaya material karton, die cutting, cetak, lipat, lem, finishing, dan packing.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="p-4 border-b flex items-center justify-between">
          <h2 className="font-semibold">Daftar Formula Harga Boks</h2>
